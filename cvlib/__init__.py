...
from nnlib.affine_align.pose_align import PoseAffineTemplate
...
def affine_align(feature_o, keypoints, align_size, template_size, Haug=None, vis=False):
    '''
    input:
        feature: float array, (H, W, 3) | (H, W)
        keypoints: normlized coordinates (N, npart*3)
    return:
        feature_rois: (N, align_size, align_size, 3) | (N, align_size, align_size) | 
        Hs: (N, 2, 3) # (H, W) -> (template_size, template_size)
    '''
    H_feat, W_feat, _ = feature_o.shape
    offset = (align_size-template_size)/2
    H_offset = np.array([[1.0, 0.0, offset], [-0.0, 1.0, offset]])
    
    N = len(keypoints)
    npart = int(keypoints.shape[-1]/3)
    keypoints_f = keypoints.copy()
    for i in range(npart):
        keypoints_f[:,i*3+0] *= W_feat
        keypoints_f[:,i*3+1] *= H_feat
        
    Hs = []
    feature_rois = []
    template = PoseAffineTemplate(npart, template_size, template_size)
    # toplots = template.drawTemplates()
    # for toplot in toplots:
    #     plt.imshow(toplot)
    #     plt.show()
    
    for keypoint in keypoints_f:
        feature = feature_o.copy()
        if vis:
            feature = template.drawKpts(feature, keypoint, npart, circle = 5) 
        ...
        if Haug is not None: # (2, 3)
            H = Haug.dot(np.vstack((H, np.array([0,0,1], dtype=np.float32))))
        Hs.append(H)
        feature_roi = cv2.warpAffine(feature, H, (align_size, align_size), borderValue=0)#, flags=cv2.INTER_CUBIC) 
        feature_rois.append(feature_roi)
    feature_rois = np.array(feature_rois)
    Hs = np.array(Hs)
    return feature_rois, Hs
