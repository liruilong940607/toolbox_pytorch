...
def heatmaps_to_keypoints(maps, rois):
    ...
    min_size = CONFIG.KRCNN.INFERENCE_MIN_SIZE
    xy_preds = np.zeros(
        (len(rois), 4, CONFIG.KRCNN.NUM_KEYPOINTS), dtype=np.float32)
    for i in range(len(rois)):
        ...
        w = roi_map.shape[2]
        for k in range(CONFIG.KRCNN.NUM_KEYPOINTS):
...
def keypoints_to_heatmap_labels(keypoints, rois):
    ...
    assert keypoints.shape[2] == CONFIG.KRCNN.NUM_KEYPOINTS

    shape = (len(rois), CONFIG.KRCNN.NUM_KEYPOINTS)
    heatmaps = blob_utils.zeros(shape)
    weights = blob_utils.zeros(shape)

    offset_x = rois[:, 0]
    offset_y = rois[:, 1]
    scale_x = CONFIG.KRCNN.HEATMAP_SIZE / (rois[:, 2] - rois[:, 0])
    scale_y = CONFIG.KRCNN.HEATMAP_SIZE / (rois[:, 3] - rois[:, 1])
    ...
    for kp in range(keypoints.shape[2]):
        ...
        if len(x_boundary_inds) > 0:
            x[x_boundary_inds] = CONFIG.KRCNN.HEATMAP_SIZE - 1

        y = (y - offset_y) * scale_y
        y = np.floor(y)
        if len(y_boundary_inds) > 0:
            y[y_boundary_inds] = CONFIG.KRCNN.HEATMAP_SIZE - 1

        valid_loc = np.logical_and(
            np.logical_and(x >= 0, y >= 0),
            np.logical_and(
                x < CONFIG.KRCNN.HEATMAP_SIZE, y < CONFIG.KRCNN.HEATMAP_SIZE))

        valid = np.logical_and(valid_loc, vis)
        valid = valid.astype(np.int32)

        lin_ind = y * CONFIG.KRCNN.HEATMAP_SIZE + x
