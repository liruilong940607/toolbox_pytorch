...
class ResnetXtBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, outplanes, stride=1, widen_factor=1, has_shortcut=False, cardinality=1, base_width=64):
        ...
        self.conv1 = nn.Conv2d(inplanes, D, kernel_size=1, stride=1, padding=0, bias=False)
...
def resnet101(pretrained=False, num_classes=None):
    ...
    if pretrained:
        def convert_resnet101():
            ...
            torch.save(model_dict, 'resnet101_form_modelzoo.pth')
            
        init_with_pretrain(model, torch.load('basemodel/resnet101_form_modelzoo.pth'))
    return model
'''
...
if __name__ == '__main__':
    ...
    backbone_net = resnet101(pretrained=False, num_classes=1000).cuda()
